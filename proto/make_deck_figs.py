...
_parser.add_argument('--only', nargs='+', choices=['calibration', 'push', 'withheld', 'madrid', 'liquid', 'strategy'])
_selected = set(_parser.parse_args().only or ['calibration', 'push', 'withheld', 'madrid', 'liquid', 'strategy'])

if 'calibration' in _selected:
...
if 'push' in _selected:
    ...
    ax.set_ylabel('Energy through the tyre, MJ per lap of age\n(within-run trend, Friday long runs)', fontsize=9); ax.set_title('Within-run energy trends for issued and withheld compounds', fontsize=10, loc='left')
    fig.tight_layout(); fig.savefig('out/fig_push.png', bbox_inches='tight')


if 'withheld' in _selected:
    ...
    ax.set_xlabel('Race degradation, s/lap per lap of age'); ax.set_title(f"Race-derived pace-loss references for {len(w)} withheld cases", fontsize=10, loc='left')
    fig.tight_layout(); fig.savefig('out/fig_withheld.png', bbox_inches='tight')


if 'madrid' in _selected:
...
if 'strategy' in _selected:
    # 6. strategy replay bars
    S = L['strategy']; evs = [e for e, rec in S.items() if all(rec.get('views', {}).get(v, {}).get('cost_under_truth_vs_best_s') is not None for v in ('Orb v1', 'Naive fit'))]
    ...
    ax.set_xticks(x); ax.set_xticklabels(evs, fontsize=8.5, rotation=25, ha='right'); ax.set_ylabel('Race time lost vs the best plan (s)'); ax.legend(fontsize=9)
    ax.set_title('Held-out strategy replay under a post-race reference model', fontsize=10, loc='left')
