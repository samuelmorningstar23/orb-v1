...
try:
 with sync_playwright() as p:
  ...
  R.navigate(page,'http://localhost:8502','/live?ev=Monza&drv=NOR&lap=32&mode=live','live')
  rep['initial']=page.locator('[data-orb-header]').first.inner_text()
  page.get_by_role('button',name='Start replay',exact=True).click();page.wait_for_timeout(15000);page.wait_for_timeout(15000)
  rep['final']=page.locator('[data-orb-header]').first.inner_text();assert 'Lap 53/53' in rep['final'],rep['final']
...
rep.update(console_errors=errors,network_failures=bad,external_requests=external);(out/'focused_report.json').write_text(json.dumps(rep,indent=2));print(json.dumps({k:v for k,v in rep.items() if k!='twin'},indent=2));sys.exit(0 if rep['passed'] else 2)
