    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            def go(path,marker):
                ...
                page.goto(args.base+path,wait_until='domcontentloaded');page.wait_for_selector(f'[data-orb-ready="{marker}"]',state='attached',timeout=30000)
            ...
            frame.locator('#orb-play').click();page.wait_for_timeout(5000);frame.locator('#orb-play').click();wait_to(205)
...
def twin_snapshot(page,server_proto,out,name):
    ...
    sampled=f.evaluate("""()=>({t:__orbTwin.t,state:__orbTwin.state(),frames:__orbTwin.frames,fps:__orbTwin.fps,drawn:__orbTwin.drawn,
      text:document.body.innerText,hud:Object.fromEntries(['orb-lapno','orb-clock','orb-gap','orb-gapm','orb-a-txt','orb-g-txt','orb-lapval','orb-fps'].map(id=>[id,document.getElementById(id).textContent]))})""")
    ...
    if source is None or source.to_dict()!=data:raise RuntimeError('RaceTwin embedded frames differ from frozen source asset')
    ...
    return dict(sample=sampled,checks=checks,numeric_audit=rec,source_frames_exact=True,embedded_frames_sha256=digest(json.dumps(data,sort_keys=True).encode()),frame_evidence=name+'_race_twin_frames.json')
