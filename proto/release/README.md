...
## Ownership map (release/ownership_audit.py; lead decisions 12 Sep 14:55)
...
| Workstream 2 | counterfactual/, events/, out/counterfactual/ |
| Workstream 3 | evaluation/ (except the sealed manifest pair), out/validation/ |
| Workstream 4 | replay/, dashboard/components/, out/maps/, app_v2/components/race_twin/ |
| Workstream 5 | interaction/ |
| Workstream 6 | app_v2/ (except components/race_twin/), ui/, theme/, views/, tests/ui/, tests/screenshots/ |
| Workstream 7 | evaluation/red_team/ |
| Workstream 8 | live/, decision/, out/live/ |
...
| Lead only (flagged when changed) | app.py, pipeline.py, model_v2.py, strategy2.py, liquid.py, refresh.sh, extract_*.py, out/lock.json, evaluation/holdout/sealed_holdout_manifest.json and .sha256 |
