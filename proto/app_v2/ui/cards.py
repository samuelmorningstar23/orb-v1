...
def kpi_html(label: str, value: str, sub: str = '', tone: str = 'neutral', source: str = '', unit: str = '') -> str:
    ...
    return (f'<div class="cs-card kpi {esc(tone)}" role="group" aria-label="{esc(label)}"><div class="cs-kpi-label">{esc(label)}</div>'
...
def kv_html(pairs, stack: bool = False) -> str:
    ...
    return f'<div class="cs-kv {"stack" if stack else ""}">' + ''.join(f'<div class="k">{esc(k)}</div><div class="v">{esc(v)}</div>' for k, v in pairs) + '</div>'
